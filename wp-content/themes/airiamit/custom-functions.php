...
// Register custom_listings Type
function init_custom_post_types() {

	$labels = array(
		'name'                  => _x( 'Pre-construction Projects', 'Post Type General Name', 'airiamit' ),
		'singular_name'         => _x( 'Pre-construction Project', 'Post Type Singular Name', 'airiamit' ),
	...
	$args = array(
		'label'                 => __( 'Pre-construction Project', 'airiamit' ),
		'description'           => __( 'Pre-construction Projects / Listings', 'airiamit' ),
		'labels'                => $labels,
	...
	$labels = array(
		'name'                  => _x( 'Listings', 'Post Type General Name', 'airiamit' ),
		'singular_name'         => _x( 'Listing', 'Post Type Singular Name', 'airiamit' ),
		'menu_name'             => __( 'Listings', 'airiamit' ),
		'name_admin_bar'        => __( 'Listings', 'airiamit' ),
		'archives'              => __( 'Listings', 'airiamit' ),
		'attributes'            => __( 'Item Attributes', 'airiamit' ),
		'parent_item_colon'     => __( 'Parent Item:', 'airiamit' ),
		'all_items'             => __( 'All Items', 'airiamit' ),
		'add_new_item'          => __( 'Add New Item', 'airiamit' ),
	...
	$args = array(
		'label'                 => __( 'Listing', 'airiamit' ),
		'description'           => __( 'Listings', 'airiamit' ),
		'labels'                => $labels,
		'supports'              => array( 'title', 'editor', 'thumbnail', 'custom-fields','post-attributes'),
		'hierarchical'          => false,
		'public'                => true,
		...
		'exclude_from_search'   => false,
		'publicly_queryable'    => true,
		'rewrite' => array('slug' => 'custom-listings'),
		'capability_type'       => 'page',
	);
	register_post_type( 'custom_listings', $args );
...
// custom_listings output
function custom_listings_output( $atts, $content ) {
	$atts = shortcode_atts(array(
			'posts' => -1,
			'show_filters' => "false",
			'layout' => 'grid',
	...
	$output = "";
	$output .= '<div class="custom-listings '.$atts['layout'].' vc_col-sm-12" data-filter-location="">';
	if(count($projects) > 0){
		$output .= '<div class="ajax-loader">
				<img src="'.get_template_directory_uri().'/assets/images/ajax-loader.gif" title="Loading..." alt="Loading..."/>
			</div>';
		$output .= '<div class="custom-listings-list">';
		/*if($atts['show_filters'] == "true"){
			$fieldCities = get_field_object('cl_location', $projects[0]->ID);
			$fieldCities = $fieldCities['choices'];
...
add_shortcode( 'custom_listings', 'custom_listings_output');

function custom_listings_filters_output(){
	$args = array(
			'post_type' => 'custom_listings',
			'posts_per_page' => 1,
			'offset' => 0,
	...
	if(count($projects) > 0){
		...
		$output .= '<div class="project-filters-external"> <div class="filter-toggle">Filter <i class="fas fa-filter"></i></div>';
		$fieldBedrooms = get_field_object('cl_bedrooms', $projects[0]->ID);
		$fieldBedrooms = $fieldBedrooms['choices'];
		$fieldBathrooms = get_field_object('cl_bathrooms', $projects[0]->ID);
		$fieldBathrooms = $fieldBathrooms['choices'];
		$fieldTypes = get_field_object('cl_type', $projects[0]->ID);
		$fieldTypes = $fieldTypes['choices'];
		$fieldPrices = get_field_object('cl_price_range', $projects[0]->ID);
		$fieldPrices = $fieldPrices['choices'];
...
add_shortcode( 'custom_listings_filters', 'custom_listings_filters_output');

function custom_listings_ajax_output(){
	$show_filters = false;
	$totalResults = 0;
	$args = array(
			'post_type' => 'custom_listings',
...
add_action("wp_ajax_cl_loadmore_listings", "custom_listings_ajax_output");
add_action("wp_ajax_nopriv_cl_loadmore_listings", "custom_listings_ajax_output");

add_filter( 'manage_custom_listings_posts_columns', 'add_custom_column' );
function add_custom_column( $columns ) {
	$columns['cl_is_featured'] = 'Is Featured?';
	return $columns;
}

add_action( 'manage_custom_listings_posts_custom_column' , 'add_custom_column_data', 10, 2 );
function add_custom_column_data( $column, $post_id ) {
	switch ( $column ) {
			case 'cl_is_featured' :
					echo(the_field('cl_is_featured', $post_object->ID));
					break;
